...
- **브랜치 이름 설정:** Git 브랜치를 생성할 때는 항상 변경 사항의 목적을 명확히 나타내는 설명적인 브랜치 이름(descriptive branch names)을 사용해라. 브랜치 이름은 반드시 feat/, fix/, refactor/, chore/ 중 하나의 접두사로 시작해야 하며, 영문 소문자와 하이픈(-)만 사용해라.

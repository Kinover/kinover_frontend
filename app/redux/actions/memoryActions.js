...
import {SET_LOADING, FETCH_MEMORY, SET_ERROR} from './actionTypes'; // actionTypes 임포트
import {getToken} from '../../utils/storage';

// 비동기 액션 생성 함수
export const fetchMemory = familyId => {
  return async dispatch => {
    ...
    try {
      ...
      const apiUrl =
        Platform.OS === 'android'
          ? `http://10.0.2.2:9090/api/memory/${familyId}` // Android용 주소
          : `http://localhost:9090/api/memory/${familyId}`; // 기타 플랫폼용 주소
      ...
      console.log('memory 데이터->', response.data);
      // 가져온 데이터로 상태 업데이트
      dispatch({
        type: FETCH_MEMORY,
